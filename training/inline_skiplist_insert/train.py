...
# We use piecewise linear model here
# n = piecewise model number
n = 4

# Functions to filter out outliers
def filter_outliers_by_x(df):
    def filter_group(group):
        ...
        return group[np.abs(group['y'] - mean_y) <= 3 * std_y]
...
data_filename = data_path + "/curOpLatencyCollect_Inline_Skiplist_Insert.csv"
model_filename = model_path + "/Inline_Skiplist_Insert.model"
...
header = lines[0]
if header != "Inline_Skiplist_Insert":
    raise ValueError("Data is not for insert into an InlineSkipList. Please check the source file!!!")
...
# Step 2.7 sample at most 100000 data for each unique x
sampled_data = (
...
# Step 2.8 Calculate the interval of each linear function
interval = max(x) / n
print(f"For each model, we have x interval of {interval}")

# Step 3: Now fit the model for each interval
plt.figure(figsize=(10, 6))
with open(model_filename, "w") as m:
    # write model type first
    m.write("Piecewise\n")

    # now train for each single model
    startXVal = 0
    endXVal = startXVal + interval
    for i in range(n):
        filtered = sampled_data[(sampled_data['x'] > startXVal) & (sampled_data['x'] <= endXVal)]
        cur_x = filtered['x']
        cur_y = filtered['y']
        ...
        # Fit the model
        params, cov = curve_fit(model_function, cur_x, cur_y, bounds=([0, -np.inf], [np.inf, np.inf]))
        a_hat, c_hat = params
        m.write(f"{a_hat}\n{0.0}\n{c_hat}\n")
        print(f"For x = ({startXVal}, {endXVal}], our Fitted parameters: a={a_hat:.4f}, c={c_hat:.4f}")
        # Plot fitted line here
        x_sorted = np.sort(np.unique(cur_x))
        plt.plot(x_sorted, model_function(x_sorted, *params), color="red", lw=2, label=f"Fitted model for x = ({startXVal}, {endXVal}]", zorder=4)
        startXVal = endXVal
        endXVal = endXVal + interval
...
#   fitted function are ploted above already
x = np.array(x)
y = np.array(y)
