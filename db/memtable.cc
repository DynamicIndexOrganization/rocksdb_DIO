...
/* Structure to collect memtable workload statistics */
MemTableStat *curGlobalStat = nullptr;

#ifdef DIO_LATENCY_COLLECT
/* Structure to collect latency information */
OpLatencyCollect *curGlobalLatencyCollect[MAX_OP_LATENCY_TYPE] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};
#endif

/* Latency Prediction Model matrix generated by training on pre-collected latency data */
LatencyModel* latencyModels[MAX_OP_LATENCY_TYPE] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};
...
ImmutableMemTableOptions::ImmutableMemTableOptions(
    ...
    : arena_block_size(mutable_cf_options.arena_block_size),
      ...
      enable_dynamic_index_organization(mutable_cf_options.enable_dynamic_index_organization),
      bucket_count(ioptions.bucket_count),
      skiplist_branching_factor(ioptions.skiplist_branching_factor),
      dynamic_index_organization_cost_adjust_factor(mutable_cf_options.dynamic_index_organization_cost_adjust_factor) {}

std::string GetOpLatencyTypeString(OpLatencyType type) {
  switch (type) {
    case VECTOR_INSERT:
    ...
    case VECTOR_SORT:
      return "Vector_Sort";
    case INLINE_SKIPLIST_INSERT:
      return "Inline_Skiplist_Insert";
    case INLINE_SKIPLIST_SEARCH:
      return "Inline_Skiplist_Search";
    case SKIPLIST_SEARCH:
      return "Skiplist_Search";
    case SKIPLIST_INSERT:
      return "Skiplist_Insert";
    case HASH_PROBE:
      return "Hash_Probe";
    default:
      return "";
...
std::string GetLatencyModelTypeString(LatencyModelType type) {
  switch (type) {
    case CONSTANT:
      return "Constant";
    case N:
...
LatencyModelType GetLatencyModelTypeFromString(std::string str) {
  for (int i = CONSTANT; i < MAX_MODEL_TYPE; i++) {
    LatencyModelType curType = (LatencyModelType)i;
    if (str == GetLatencyModelTypeString(curType)) {
      return curType;
    }
  }
  return MAX_MODEL_TYPE;
}

LatencyModel *LoadPiecewiseLatencyPredictModels(std::ifstream &file) {
  ...
  while (std::getline(file, line)) {
    if (readType) {
      ...
      range.push_back(stod(line));
      readRange = false;
    } else {
      // Read three parameters now
      params[index].push_back(stod(line));
      index++;
      if (index > 2) {
        // Finish reading the current model. Read the next here.
        index = 0;
        readType = true;
        readRange = true;
      }
  ...
  return new PiecewiseLatencyModel(params[0], params[1], params[2], type, range);
}

LatencyModel *LoadSingleLatencyPredictModels(std::string modelType, std::ifstream &file) {
  std::vector<double> params;
  std::string line = "";
  while (std::getline(file, line)) {
    params.push_back(stod(line));
  }
  return new LatencyModel(modelType, params);
}

void MemTable::LoadLatencyPredictModels() {
  std::string modelDirPath = std::getenv("LATENCY_PREDICT_MODEL_PATH");
  if (modelDirPath == "") {
    printf("Failed to load latency prediction model path in env. Please define the environment variable LATENCY_PREDICT_MODEL_PATH\n");
    return;
  }

  for (int type = VECTOR_INSERT; type < MAX_OP_LATENCY_TYPE; type++) {
    std::string modelFilePath = modelDirPath + "/" + GetOpLatencyTypeString((OpLatencyType)type) + ".model";
    std::ifstream file;
    ...
    while (std::getline(file, line)) {
      ...
      if (modelType == "Piecewise") {
        latencyModels[type] = LoadPiecewiseLatencyPredictModels(file);
      } else {
        latencyModels[type] = LoadSingleLatencyPredictModels(modelType, file);
      }
    }
    latencyModels[type]->PrintModel(type);
  }
  fprintf(stderr, "DIODEBUG: DynamicIndexOrganization is Enabled!!!\n");
}

static double CalculateCumulatedLogCost(double x,  size_t log_base, LatencyModel *model) {
  double logX = std::ceil(std::log2(x) / std::log2(log_base));
  double i = 1;
  double cost = 0;
  double xLeft = x;
  while (i <= logX) {
    double curX = pow(log_base, i) - pow(log_base, i-1);
    curX = (curX > xLeft)? xLeft:curX;
    cost += curX * model->Predict(i);
    xLeft -= curX;
    i++;
  }
...
double MemTable::PredictVectorMemtableCost(MemTableStat& allMemStat, bool printOut) {
  // For VectorInsert, total cost = num of records * single insert latency
  double insertCost = allMemStat.numInsertRecords * latencyModels[VECTOR_INSERT]->Predict();
  // For VectorGet, total cost = num of get * (copy of vector + vector sort + binary search)
  //     binary search cost is not significant here, so we ignore it for now.
  double copySortCost = latencyModels[VECTOR_COPY]->Predict(allMemStat.numInsertRecords) +
                         latencyModels[VECTOR_SORT]->Predict(allMemStat.numInsertRecords);
  double pointReadCost = allMemStat.numPointRead * copySortCost;
  // For VectorScan, total cost is the same as doing the point read since both
  // performs a seek, which dominant the whole performance
  double rangeScanCost = allMemStat.numRangeQuery * copySortCost;

  double totalCost = insertCost + pointReadCost + rangeScanCost;
  if (printOut)
    printf("Type: %s:\n  InsertCost = %f\n  pointReadCost = %f\n  rangeScanCost = %f\n    totalCost = %f\n",
      "Vector", insertCost, pointReadCost, rangeScanCost, totalCost);
  return totalCost;
}

double MemTable::PredictSkipListMemtableCost(MemTableStat& allMemStat, size_t skiplist_branching_factor, bool printOut) {
  // For InlineSkipListInsert, total cost = num of records * single insert latency
  //   single insert latency is calculated based on the height of SkipList, which
  //   is estimated using numInsertRecords.
  uint64_t curNumEntry = (allMemStat.curMemTableNumEntries == 0)? (uint64_t)allMemStat.numInsertRecords:allMemStat.curMemTableNumEntries;
  double estHeight = std::ceil(std::log2((double)curNumEntry) / std::log2(skiplist_branching_factor));
  double insertCost = CalculateCumulatedLogCost(allMemStat.numInsertRecords, skiplist_branching_factor, latencyModels[INLINE_SKIPLIST_INSERT]);
  // For InlineSkipListGet, total cost = num of get * single skiplist search
  double pointReadCost = allMemStat.numPointRead * latencyModels[INLINE_SKIPLIST_SEARCH]->Predict(estHeight);
  // For InlineSkipListScan, total cost is the quite similar to pointReadCost (for now)
  // due to the sorted nature of the last layer of skiplist.
  double rangeScanCost = allMemStat.numRangeQuery * latencyModels[INLINE_SKIPLIST_SEARCH]->Predict(estHeight);

  double totalCost = insertCost + pointReadCost + rangeScanCost;
  if (printOut)
    printf("Type: %s:\n  InsertCost = %f\n  pointReadCost = %f\n  rangeScanCost = %f\n    totalCost = %f\n",
      "SkipList", insertCost, pointReadCost, rangeScanCost, totalCost);
  return totalCost;
}

double MemTable::PredictHashSkipListMemtableCost(MemTableStat& allMemStat, size_t bucket_count, size_t skiplist_branching_factor, bool printOut) {
  // For HashSkipListInsert, total cost = num of records * single insert latency
  //   single insert latency is calculated based on the height of SkipList, which
  //   is estimated using numInsertRecords/bucket_count.
  uint64_t curNumEntry = (allMemStat.curMemTableNumEntries == 0)? (uint64_t)allMemStat.numInsertRecords:allMemStat.curMemTableNumEntries;
  double numRecordsInBucket = (double)(curNumEntry/bucket_count);
  double estHeight = std::ceil(std::log2(numRecordsInBucket) / std::log2(skiplist_branching_factor));
  if (estHeight < 1) {
    estHeight = 1;
  }
  double insertCost = allMemStat.numInsertRecords * (latencyModels[HASH_PROBE]->Predict() + latencyModels[SKIPLIST_INSERT]->Predict(estHeight));
  assert(insertCost >= 0);
  // For HashSkipListGet, total cost = num of get * single skiplist search
  double pointReadCost = allMemStat.numPointRead * (latencyModels[HASH_PROBE]->Predict() + latencyModels[SKIPLIST_SEARCH]->Predict(estHeight));
  // For HashSkipListScan, total cost is different since it tries to create a tempory SkipList by scanning
  // through multiple buckets. Currently, we estimate the Scan selectivity to be 50%.
  double numScanRecords = allMemStat.numInsertRecords/2;
  double tempHeight = std::log(numScanRecords);
  double singleScanCost = bucket_count * latencyModels[SKIPLIST_SEARCH]->Predict(estHeight) +
                          numScanRecords * latencyModels[SKIPLIST_INSERT]->Predict(tempHeight);
  double rangeScanCost = allMemStat.numRangeQuery * singleScanCost;

  double totalCost = insertCost + pointReadCost + rangeScanCost;
  if (printOut)
    printf("Type: %s:\n  InsertCost = %f\n  pointReadCost = %f\n  rangeScanCost = %f\n    totalCost = %f\n",
      "HashSkipList", insertCost, pointReadCost, rangeScanCost, totalCost);
  return totalCost;
}

bool MemTable::NeedSwitchNewMemtable() {
  // return immediately if we don't need a new memtable yet
  if ((!moptions_.enable_dynamic_index_organization) ||
      curGlobalStat == nullptr ||
      !curGlobalStat->WorkloadStarted() ||
      ((fast_rand() % 100) < 10)) {
    return false;
  }

  // Check if latency prediction models are loaded
  if (latencyModels[0] == nullptr) {
    LoadLatencyPredictModels();
  }

  // Check curGlobalStat condition here to see if we need to proceed
  // Condition: 1seconds = 1000000000 nanosecond OR
  //            10000 query executed
  if (!curGlobalStat->CheckQueryNumber(10000) && !curGlobalStat->CheckTimeExpired(1000000000)) {
    curGlobalStat->SetCheckTime();
    return false;
  }

  // Calculate costs of different models and decide the lowest one
  MemTableType type = curGlobalStat->type;
  double curMinCost = DBL_MAX;
  if (false && curGlobalStat->IsInsertOnly()) {
    type = VECTOR_TYPE;
  } else if (false && curGlobalStat->IsFullyMixedWorkload()) {
    type = SKIP_LIST_TYPE;
  } else {
    // calculate the cost of current table type here first
    type = curGlobalStat->type;
    switch (type) {
      case SKIP_LIST_TYPE:
        curMinCost = PredictSkipListMemtableCost(*curGlobalStat, moptions_.skiplist_branching_factor, false);
        break;
      case VECTOR_TYPE:
        curMinCost = PredictVectorMemtableCost(*curGlobalStat, false);
        break;
      case HASH_SKIP_LIST_TYPE:
        curMinCost = PredictHashSkipListMemtableCost(*curGlobalStat, moptions_.bucket_count, moptions_.skiplist_branching_factor, false);
        break;
      default:
        std::cerr << "DIOLOG: WRONG MemTable Type" << std::endl;
        break;
    }

    // now calculate the cost of the rest options
    for (int i = 0; i < MAX_SUPPORTED_MEM_TABLE_TYPE; i++) {
      double curCost = DBL_MAX;
      MemTableType curType = (MemTableType)i;
      if (curType == curGlobalStat->type) {
        continue;
      }
      switch (curType) {
        case SKIP_LIST_TYPE:
          curCost = PredictSkipListMemtableCost(*curGlobalStat, moptions_.skiplist_branching_factor, false);
          break;
        case VECTOR_TYPE:
          curCost = PredictVectorMemtableCost(*curGlobalStat, false);
          break;
        case HASH_SKIP_LIST_TYPE:
          curCost = PredictHashSkipListMemtableCost(*curGlobalStat, moptions_.bucket_count, moptions_.skiplist_branching_factor, false);
          break;
        default:
          std::cerr << "DIOLOG: WRONG MemTable Type" << std::endl;
          break;
      ...
      // we introduce a cost asjust factor here to prevent frequent memtable implementation occilation.
      if (curCost < (curMinCost * moptions_.dynamic_index_organization_cost_adjust_factor)) {
        type = curType;
        curMinCost = curCost;
      }
    }
  ...
  // return true if the optimal memtable implementation is different from the one in-use
  bool result = (type != curGlobalStat->type);
  if (result) {
    printf("DIODEBUG: Need to switch memtable from %d to %d\n", curGlobalStat->type, type);
  }

  // no need to reset global stat if workload shift, just in case
  // since it will be used to calculate workload cost later when memtable switch
  if (!result) {
    curGlobalStat->Init(curGlobalStat->type);
  } else {
    // if the type changes, give a hint about which type to use for the memtable creation later.
    curGlobalStat->nextMemTableType = type;
  }

  return result;
}

void MemTable::PrintTimeToLive(std::string reason) {
  auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - create_time);
  (void)duration;
  (void)reason;
  // printf("DIODEBUG: This memtable has lived for %ld ns before it needs to be flushed due to %s.\n", duration.count(), reason.c_str());
}

MemTableRep* MemTable::CreateTableInMemory(const KeyComparator &comparator, ConcurrentArena *arena,
    const MutableCFOptions& mutable_cf_options, const ImmutableOptions& ioptions, uint32_t column_family_id)
{
  // Return immediately if dynamic index organization is not enabled
  if (!mutable_cf_options.enable_dynamic_index_organization) {
#ifdef DIO_LATENCY_COLLECT
    // create the curGlobalLatencyCollect object if not yet
    // Only create it when we enabled the latency collection mode
    if (curGlobalLatencyCollect[0] == nullptr) {
      for (int i = VECTOR_INSERT; i < MAX_OP_LATENCY_TYPE; i++) {
        curGlobalLatencyCollect[i] = new OpLatencyCollect((OpLatencyType)i, false);
      }
    }
#endif
    return ioptions.memtable_factory->CreateMemTableRep(
        comparator, arena, mutable_cf_options.prefix_extractor.get(),
  ...
  // Calculate costs here and decide which memtable implementation we want to use
  // just use all information in curGlobalStat to calculate cost
  MemTableType type = (curGlobalStat == nullptr)? ioptions.memtable_factory->type:curGlobalStat->type;
  double curMinCost = DBL_MAX;
  if (curGlobalStat != nullptr && curGlobalStat->WorkloadStarted() && curGlobalStat->TotalQueryNumber() > 100) {
    if (curGlobalStat->nextMemTableType != MAX_MEM_TABLE_TYPE) {
      // The early workload shift detection has already give us a hint about which memtable to switch to.
      // No need to perform the cost calculation again
      type = curGlobalStat->nextMemTableType;
    } else {
      // Otherwise, just perform another check here
      MemTableStat allMemStat;
      allMemStat.numInsertRecords = curGlobalStat->numInsertRecords.load(std::memory_order_relaxed);
      allMemStat.numRangeQuery = curGlobalStat->numRangeQuery.load(std::memory_order_relaxed);
      allMemStat.numPointRead = curGlobalStat->numPointRead.load(std::memory_order_relaxed);
      allMemStat.curMemTableNumEntries = curGlobalStat->curMemTableNumEntries;
      // for insert only workload, Let's just use vector.
      if (false && allMemStat.IsInsertOnly()) {
        type = VECTOR_TYPE;
      } else if (false && allMemStat.IsFullyMixedWorkload()) {
        type = SKIP_LIST_TYPE;
      } else {
        // calculate the cost of current table type here first
        switch (type) {
          case SKIP_LIST_TYPE:
            curMinCost = PredictSkipListMemtableCost(allMemStat, ioptions.skiplist_branching_factor, false);
            break;
          case VECTOR_TYPE:
            curMinCost = PredictVectorMemtableCost(allMemStat, false);
            break;
          case HASH_SKIP_LIST_TYPE:
            curMinCost = PredictHashSkipListMemtableCost(allMemStat, ioptions.bucket_count, ioptions.skiplist_branching_factor, false);
            break;
          default:
            std::cerr << "DIOLOG: WRONG MemTable Type" << std::endl;
            break;
        ...
        for (int i = 0; i < MAX_SUPPORTED_MEM_TABLE_TYPE; i++) {
          ...
          switch (curType) {
            case SKIP_LIST_TYPE:
              curCost = PredictSkipListMemtableCost(allMemStat, ioptions.skiplist_branching_factor, false);
              break;
            case VECTOR_TYPE:
              curCost = PredictVectorMemtableCost(allMemStat, false);
              break;
            case HASH_SKIP_LIST_TYPE:
              curCost = PredictHashSkipListMemtableCost(allMemStat, ioptions.bucket_count, ioptions.skiplist_branching_factor, false);
              break;
            default:
              std::cerr << "DIOLOG: WRONG MemTable Type" << std::endl;
              break;
          ...
          // we introduce a cost asjust factor here to prevent frequent memtable implementation occilation.
          if (curCost < (curMinCost * mutable_cf_options.dynamic_index_organization_cost_adjust_factor)) {
            type = curType;
            curMinCost = curCost;
          }
        }
      }
      assert(type < MAX_SUPPORTED_MEM_TABLE_TYPE);
    }
  } else {
    // First memtable ever since workload haven't started yet, just create one based on the user option
  }

  if (curGlobalStat == nullptr) {
    curGlobalStat = new MemTableStat();
    curGlobalStat->Init(type);
  } else {
    curGlobalStat->type = type;
  }

  MemTableRepFactory *curFactory = ioptions.memtable_factory.get();
  switch (type) {
    case SKIP_LIST_TYPE:
      curFactory = ioptions.skip_list_memtable_factory.get();
      break;
    case VECTOR_TYPE:
      curFactory = ioptions.vector_memtable_factory.get();
      break;
    case HASH_SKIP_LIST_TYPE:
    case HASH_LINKED_LIST_TYPE:
      curFactory = ioptions.hash_skip_list_memtable_factory.get();
      break;
    default:
      std::cerr << "DIOLOG: WRONG MemTable Type" << std::endl;
      assert(0);
  ...
  curFactory->PrintMemTableType();
  rocksdb::MemTableRep *rep = nullptr;
  const SliceTransform *prefix_extractor = mutable_cf_options.prefix_extractor.get();
  rep = curFactory->CreateMemTableRep(comparator, arena, prefix_extractor, ioptions.logger, column_family_id);
  ...
  // Reset curGlobalStat here if needed.
  // Condition: 1seconds = 1000000000 nanosecond OR
  //            10000 query executed
  if (curGlobalStat != nullptr && (curGlobalStat->CheckQueryNumber(10000) || curGlobalStat->CheckTimeExpired(1000000000))) {
    curGlobalStat->Init(curFactory->type);
  }
...
MemTable::MemTable(const InternalKeyComparator& cmp,
    ...
    : comparator_(cmp),
      ...
      table_(MemTable::CreateTableInMemory(comparator_, &arena_, mutable_cf_options, ioptions, column_family_id)),
      ...
      locks_(moptions_.inplace_update_support
                 ...
                 : 0),
      prefix_extractor_(table_->GetTransformer()),
...
MemTable::~MemTable() {
  ...
  assert(refs_ == 0);
  // DIONOTE: Disable for now to bypass a segfault issue
  // if (oldMemTable) {
  //   delete oldMemTable;
  //   oldMemTable = nullptr;
  // }
...
bool MemTable::ShouldSwitchMemtableRep() {
  // Only allow one memtable conversion to the active memtable. Do not convert a converted memtable.
  // Also, do not convert memtable if the current workload is Write-only, as it will not read
  // any existing tables.
  if (oldMemTable != nullptr || (curGlobalStat != nullptr && curGlobalStat->IsInsertOnly())) {
    return false;
  }
  ...
  return (allocated_memory < 0.85 * write_buffer_size);
}
...
bool MemTable::ShouldFlushNow() {
  ...
  if (memtable_max_range_deletions_ > 0 &&
    ...
    PrintTimeToLive("too many range deletions");
    return true;
  }
  ...
  // if the table is already partially full, check if the current implementation is
  // sub-optimal to the current workload and do we need to switch to a new memtable
  // implementation. We need to collect some stat about the current workload to make
  // an accurate decision.
  if (NeedSwitchNewMemtable()) {
    PrintTimeToLive("the current memtable is sub-optimal to the current workload");
    return true;
  }
  ...
  if (allocated_memory >
      write_buffer_size + kArenaBlockSize * kAllowOverAllocationRatio) {
    PrintTimeToLive("user keeps adding large entries");
    return true;
  }
  ...
  if (arena_.AllocatedAndUnused() < kArenaBlockSize / 4) {
    PrintTimeToLive("last block is 75%% full");
    return true;
  }
...
InternalIterator* MemTable::NewIterator(
  ...
  // If new memtable appears, use new memtable to perform the search.
  // Otherwise, use the current memtable.
  MemTable *table = (newMemTable != nullptr)? (MemTable *)newMemTable:this;
  return new (mem)
      MemTableIterator(MemTableIterator::kPointEntries, *table, read_options,
...
void MemTable::ParseEntry(const char *entry, SequenceNumber &s, ValueType &t, char *&key, char *&value, uint32_t &keyLen, uint32_t &valueLen)
{
  // parse key first
  key = (char *)GetVarint32Ptr(entry, entry + 5, &keyLen);
  assert(keyLen >= 8);
  // key length should exclude a uint64 which packed type and sequence number together
  keyLen -= 8;
  const uint64_t tag = DecodeFixed64(key + keyLen);
  UnPackSequenceAndType(tag, &s, &t);
  // parse value now
  value = (char *)GetVarint32Ptr(key + keyLen + 8, key + keyLen + 13, &valueLen);
}

void MemTable::UpdateSequenceNumber(SequenceNumber firstSeq, SequenceNumber earliestSeq)
{
  first_seqno_ = firstSeq;
  earliest_seqno_ = earliestSeq;
}
...
Status MemTable::Add(SequenceNumber s, ValueType type,
  ...
  if (!allow_concurrent) {
    ...
    assert(first_seqno_ == 0 || s >= first_seqno_ || convertMode);
    if (first_seqno_ == 0) {
    ...
    if (!convertMode) UpdateFlushState();
  } else {
...
bool MemTable::Get(const LookupKey& key, std::string* value,
  ...
  if (bloom_filter_ && !may_contain) {
    ...
    *seq = kMaxSequenceNumber;
    ...
    MemTable * newTable = (MemTable *)newMemTable;
  // If new memtable appears, use new memtable to perform the search.
  // Otherwise, use the current memtable.
    if (newTable != nullptr) {
      // This scenario will only happen to immutable tables that is converted to another memtable type
      assert(table_->isImmutable());
      newTable->GetFromTable(key, *max_covering_tombstone_seq, do_merge, callback,
